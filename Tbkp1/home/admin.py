...
class MoveOfferInline(admin.TabularInline):
    model = MoveOffer
    extra = 0
    readonly_fields = ('mover', 'price', 'status', 'distance_km', 'eta_minutes', 'created_at')


@admin.register(MoveRequest)
class MoveRequestAdmin(admin.ModelAdmin):
    list_display = ('user', 'moving_from', 'move_date', 'status', 'request_mover', 'created_at')
    list_filter = ('status', 'request_mover', 'move_time')
    search_fields = ('user__username', 'moving_from', 'moving_to_manual')
    inlines = [MoveOfferInline]


@admin.register(MoveOffer)
class MoveOfferAdmin(admin.ModelAdmin):
