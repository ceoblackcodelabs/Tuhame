...
def estimate_eta_minutes(distance_km, avg_speed_kmh=22):
    """
    Rough ETA estimate for urban Nairobi driving conditions (traffic-adjusted
    average speed, not a real routing engine). Adds a flat 3-minute buffer.
    """
    if distance_km <= 0:
        return 3
    return round((distance_km / avg_speed_kmh) * 60) + 3
